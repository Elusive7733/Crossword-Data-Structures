...
//		system("cls");
		...
		for (int i = 0 ; i < y_max ; i++){
			...
			for (int j = -1 ; j < x_max ; j++){
				if (j != -1){
					if ((smallArray[i][j] >= 'a' && smallArray[i][j] <= 'z') && (smallTemparray[i][j] == '-')){
						cout<<"-  ";
					}
					else if((smallArray[i][j] >= 'a' && smallArray[i][j] <= 'z') && (smallTemparray[i][j] == '1')){
						cout << count << "  ";
...
//					if(smallArray[i][j+1] != ' '){
//						while(smallArray[i][j+k] != ' ' && ((j+k) < x_max)){
...
//						k = 0;
//					}
//					else if(smallArray[i+1][j] != ' '){
//						while(smallArray[i+k][j] != ' ' && ((i+k) < y_max)){
