...
private:
	...
	SinglyLinkedList Starting_letters;
	SinglyLinkedList Inserted_words;
				...
				for(int j = x_min; j < x_max; j++){
					...
					for (int k = 0 ; k < database[dcount].length() ; k++){
						...
						if (largeArray[i][j] == database[dcount][k]){
							...
							if (inserted == true){
								Inserted_words.addAtTail(database[dcount]);
								break;
							}
							else{
...
//				cout<<"here1b"<<endl;
				for (int x = j - k ; x < j + len - k ; x++){
					...
					empty_char += newWord[0];
					Starting_letters.addAtTail(empty_char);
					count++;
				}
				for(int i = 0; i < 15; i++){
